...
class HexView {
    ...
    _makeRows(startLine, nLines) {
        ...
        for (let off = start; off < start+length; off += 16) {
            ...
            tr.addEventListener("click", () => {
                highlightRanges([[startAddr.toString(), nextAddr.toString()]], view);
            });
            rowAddr = nextAddr;
    ...
    highlightRanges(ranges, scroll) {
        ...
        for (let r of ranges) {
            ...
            const b1 = new AddrJS(r[0]).sub(this._addr).toNumber();
            const b2 = new AddrJS(r[1]).sub(this._addr).toNumber();
            for (let b = b1; b < b2;) {
